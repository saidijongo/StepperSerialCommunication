...
void rotate(int motorPin, int angleDegrees, bool clockwise) {
  int steps = angleDegrees / angleToSteps;
    ...
    if (!resetInProgress && !moveInProgress) {
      if (command == "C") {
        if (!leftSensorInterrupted) {
          motorStopped = false;
          moveInProgress = true;
          rotate(CW, 90, true); // Rotate 90 degrees clockwise
        ...
        if (!rightSensorInterrupted) {
          motorStopped = false;
          moveInProgress = true;
          rotate(CCW, 90, true); // Rotate 90 degrees counter-clockwise
          moveInProgress = false;
          motorStopped = true;
        }
      } else if (command == "R") {
